...
export default function SoundsAdmin() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(
      ...
      (err) => {
        console.error('[SoundsAdmin] Errore lettura generi:', err);
  ...
  const addTrack = async (gid, defaultPrice) => {
    ...
    if (!title) return;
    try {
      // Prefer server-side creation via callable to bypass Firestore rules
      try {
        const createFn = httpsCallable(functions, 'createBuyTrack');
        await createFn({ gid, title: title.trim(), defaultPrice: (typeof defaultPrice === 'number' && !Number.isNaN(defaultPrice)) ? defaultPrice : null });
      } catch (e) {
        console.warn('[SoundsAdmin] createBuyTrack callable failed, fallback to client addDoc:', e?.message || e);
      ...
      alert('Errore creazione brano: ' + (e && e.message ? e.message : ''));
    }
  };
  ...
  const uploadTrackAudio = async (gid, tid, file, onProgress) => {
    ...
    try {
      ...
      const r = ref(storage, `buy/tracks/${gid}_${tid}_${Date.now()}_${safeName}`);
      try {
        await new Promise((resolve, reject) => {
          const task = uploadBytesResumable(r, file, {
            contentType: file.type || 'audio/mpeg',
            // Keep inline for preview; download asset uses the ZIP/AUDIO uploader with attachment
        ...
        console.warn('[SoundsAdmin] resumable audio fallito, uso simple', e?.message || e);
        await uploadBytes(r, file, {
          contentType: file.type || 'audio/mpeg',
          contentDisposition: `inline; filename="${safeName}"`
        });
  ...
  const uploadTrackZip = async (gid, tid, file, setLocalTracks, onProgress) => {
    ...
    try {
      ...
      try {
        await new Promise((resolve, reject) => {
          const task = uploadBytesResumable(r, file, {
            contentType: file.type || (safeName.toLowerCase().endsWith('.zip') ? 'application/zip' : 'application/octet-stream'),
        ...
        console.warn('[SoundsAdmin] resumable zip fallito, uso simple', e?.message || e);
        await uploadBytes(r, file, {
          contentType: file.type || (safeName.toLowerCase().endsWith('.zip') ? 'application/zip' : 'application/octet-stream'),
          contentDisposition: `attachment; filename="${safeName}"`
        });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 32, padding:'24px 20px', maxWidth:1200, margin:'0 auto' }}>
      <h1 style={{ color:'#ffd700', margin:0, fontSize:'1.9rem' }}>Sounds – Admin</h1>
        ...
        <div key={t.id} style={{ background: '#111', border: '1px solid #333', borderRadius: 10, padding: 10, display: 'grid', gridTemplateColumns: '2fr 90px 1fr 1.1fr 1fr 220px 120px', gap: 8, alignItems: 'center' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:6 }}>
              <input id={`zip_${gid}_${t.id}`} type="file" accept=".zip,application/zip,audio/*" style={{ display:'none' }} onChange={async e => {
                const f = e.target.files && e.target.files[0];
                if (f) {
              ...
              <button className="dash-small-btn" style={{ background: zipUploading[t.id] ? '#444' : '#222', color:'#ffd700' }} disabled={zipUploading[t.id]} onClick={() => document.getElementById(`zip_${gid}_${t.id}`).click()}>
                {zipUploading[t.id] ? `Upload… ${zipPct[t.id] ?? 0}%` : 'File Download (ZIP/Audio)'}
              </button>
          ...
          <div style={{ gridColumn: '1 / -1', display: 'grid', gridTemplateColumns:'1fr auto', gap: 8, alignItems:'center' }}>
            <div style={{ display:'flex', gap:6, alignItems:'center' }}>
              <input type="text" value={t.stripePriceId || ''} onChange={e => setTracks(list => list.map(x => x.id === t.id ? { ...x, stripePriceId: e.target.value } : x))} placeholder="Stripe Price ID (per webhook)" style={{ padding: 10, borderRadius: 8, border: '1px solid #444', background: '#111', color: '#fff' }} />
              <span style={{ fontSize:11, color:'#888' }}>Usato dal webhook per marcare venduto</span>
            </div>
            <button
